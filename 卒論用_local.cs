...
public class grad_local : MonoBehaviour
{
    ...
    // パラメータ
    [SerializeField, Range(0, 99)] // 新たなチェックポイントの数
    public int cpNum1;
    ...
    [SerializeField, Range(0, 99)]
    public int cpNum10;
    [SerializeField, Range(0, 99)] // 新たなチェックポイントの場所
    ...
    [SerializeField, Range(1, 4)] // 新たなチェックポイントの方位（第何象限か）
    ...
    void Start()
    {
        ...
        if (!File.Exists(Path.Combine(Application.dataPath, "Logs", "param_local.csv")))
        {
            try
            {
                ...
                using (StreamWriter writer = new StreamWriter(Path.Combine(Application.dataPath, "Logs", "param_local.csv"), false))
                {
                    ...
                    writer.WriteLine($"{cpNum1},{cpNum2},{cpNum3},{cpNum4},{cpNum5},{cpNum6},{cpNum7},{cpNum8},{cpNum9},{cpNum10},{cpNum1_range},{cpNum2_range},{cpNum3_range},{cpNum4_range},{cpNum5_range},{cpNum6_range},{cpNum7_range},{cpNum8_range},{cpNum9_range},{cpNum10_range},{cpNum1_dir},{cpNum2_dir},{cpNum3_dir},{cpNum4_dir},{cpNum5_dir},{cpNum6_dir},{cpNum7_dir},{cpNum8_dir},{cpNum9_dir},{cpNum10_dir}");
                    Debug.Log($"パラメータを記録しました: param_local.csv");
                }
            }
            catch (IOException e)
            {
        ...
        Debug.Log($"strategyの長さ: {strategy.Length}");

        //// バグをアクティブ化
        //if (stuckBugs != null)
        //{
        //    stuckBugs.SetActive(true);
        //    Debug.Log("StuckBugs オブジェクトをアクティブにしました。");
        //}
        //else
        //{
        ...
        try
        {
            ...
            using (StreamWriter writer = new StreamWriter(filePath, false))
            {
                ...
                Debug.Log($"CSVファイル 'fuzz.csv' を作成しました: {filePath}");
            }
        }
        catch (IOException e)
        {
    ...
    // ログメッセージをチェックし、"バグ発生"の文字列が含まれていたらシーンリスタート
    private void OnLogMessageReceived(string condition, string stackTrace, LogType type)
    {
        // 例: 特定の文字列 "特定のメッセージ" が含まれているかをチェック
        if (condition.Contains("バグ発生"))
        {
            Debug.Log("バグが検出されました");
            // test_BugAlert内でバグ検出記録と次に行うファジングの記録をしている
            MutateParam();
            Application.logMessageReceived -= OnLogMessageReceived;
            Scene scene = SceneManager.GetActiveScene(); // 現在のシーンを取得
            SceneManager.LoadScene(scene.path); // シーンを再読み込み
    ...
    private void MutateParam()
    {
        ...
        if (nextType == "init")
        {
            Debug.Log("次は大域ファジングなので，localパラメータを初期化します．");
